...
First, ingest your documents to build the vector store:

```bash
uv run python rag.py
```

This reads the Markdown files from `docs/`, chunks and embeds them, and saves the vector store to `vector_store_metadata.json`. You only need to do this once (or again when your documents change).
